...
const logConsentToServer = async (preferences, action = 'updated') => {
  ...
  // Payload conforme au schéma CNIL
  const payload = {
    consent_id: generateUUID(),
    device_id: getDeviceId(),
    site_host: location.host,
    site_path: location.pathname,
    action: action === 'deleted' ? 'revoked' : (action === 'updated' ? 'updated' : 'created'),
    
    // Préférences (noms alignés avec colonnes SQL)
    ...
    // User-Agent (sera hashé côté serveur en ua_hash)
...
function renderOnce() {
  ...
  // Bloc logging comme catégorie cliquable (si activé)
